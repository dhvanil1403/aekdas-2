...
const addScreen = async (req, res) => {
    ...
    try {
        if (pairingCode.length > 6) {
            await logAction('addScreen', 'Failed to add screen: pairing code too long', req.ip);
            return res.render("Screen", {
                message: {
                    type: "error",
        ...
        if (existingScreen.length > 0) {
            ...
            await logAction('addScreen', 'Failed to add screen: pairing code already exists', req.ip);
            return res.render("Screen", {
                message: {
                    type: "error",
        ...
        await logAction('addScreen', 'Successfully added screen', req.ip);
        ...
        console.error(error);
        await logAction('addScreen', 'Error adding screen', req.ip);
        res.status(500).send("Add Screen error");
    }
};
...
const editScreen = async (req, res) => {
    ...
    try {
        ...
        await logAction('editScreen', `Screen edited: ${pairingCode}`, req.ip);
        ...
        console.error(error);
        await logAction('editScreen', Error `editing screen: ${pairingCode}`, req.ip);
        res.status(500).send("Error editing screen");
    }
};
...
const restoreScreen = async (req, res) => {
    ...
    try {
        const restoredScreen = await screen.restoreScreenInDB(pairingCode);
        await logAction('restoreScreen', Screen `restored: ${pairingCode}`, req.ip);
        res.json({ success: true, screen: restoredScreen });
    } catch (error) {
        console.error(error);
        await logAction('restoreScreen', Error `restoring screen: ${pairingCode}`, req.ip);
        res.status(500).json({ success: false, message: "Error restoring screen" });
    }
};
